...
void prova(const tree * node, double sum, int n_nodi, list* & result)
{
	if (node != nullptr) 
	{
		if(node->info%2 == 0)
		...
		if(node->right == nullptr && node->left == nullptr)
		{
			if(n_nodi != 0)
			{
				result = new list{sum/n_nodi, result};
			}		
		} 
		else if(node->right == nullptr)
		{
			...
			prova(node->left, sum, n_nodi, result);
		}
		else if(node->left == nullptr)
		{
			...
			prova(node->right, sum, n_nodi, result);
		}
		else
		{
			prova(node->left, sum, n_nodi, result);
