...
void get_lenght(const tree* node, int l1_size, int l2_size)
{
    if(node != nullptr)
    {
        if(node->info%2 == 0)
        ...
        {
            get_lenght(node->left, l1_size, l2_size);
        }
        
        if(node->right != nullptr)
        {
            get_lenght(node->right, l1_size, l2_size);
        }
    }
}
...
{
	l1_size = 0;
    get_lenght(root, l1_size, l2_size);
    std::cout << l1_size << std::endl << l2_size << std::endl;
}
