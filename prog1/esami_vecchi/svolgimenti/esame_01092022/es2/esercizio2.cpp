...
list* estrai_aux(const tree* root, list* result)
{
	if(root == NULL)
	{
		return result;
	...
	result = estrai_aux(root->left, result);
	if(root->info%2 == 0)
	{
		result = new list{root->info, result};
	}
	result = estrai_aux(root->right, result);
	return result;
