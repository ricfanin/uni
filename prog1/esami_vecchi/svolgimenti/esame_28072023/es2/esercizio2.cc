...
// implementare
list* recursive(float x, int N, double e, int i, float sum, list* nodo)
{
	double func = sum + pow(x, ((2*i)+1))/factorial((2*i) + 1);
	if(i <= N && (func - sum) >= (e * sum))
	{
		nodo = new list{func, nodo};
		return recursive(x, N, e, i+1, func, nodo);
	}
	else
	{
		return nodo;
